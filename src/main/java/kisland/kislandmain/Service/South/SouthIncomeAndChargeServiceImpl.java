...
@Transactional
@Service
public class SouthIncomeAndChargeServiceImpl implements SouthIncomeAndChargeService {
    @Autowired
    ...
    @Override
    public Map<String, String> getSouthIncomeByType(String startTime, String endTime,List<IncomeCategoryDO> incomeCategoryDOS) {
        ...
        List<ReceivingMappingDO> list = receivingChannelService.findByTradeScope("南区");
                                    ...
                                    if (set1.add(uuid)) {
                                        for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
                                            if (incomeCategoryDO.getCategoryName().equals(receivingMappingDO.getReceivingChannel())) {
                                                incomeCategoryDO.setSouthCategoryValue(Double.parseDouble(incomeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(chinaBankDO.getTradeAmount().replace("-", "")) + "");
                                            }
                                        }
                            ...
                            for (AgriculturalBankDO agriculturalBankDO : agriculturalBankDOList) {
                                ...
                                if (str.equals(agriculturalBankDO.getAccountTitle()) && agriculturalBankDO.getOtherPartyName().equals(receivingMappingDO.getOtherAccountName())) {
                                    if (set1.add(uuid)) {
                                        for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
                                            if (incomeCategoryDO.getCategoryName().equals(receivingMappingDO.getReceivingChannel())) {
                                                incomeCategoryDO.setSouthCategoryValue(Double.parseDouble(incomeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(agriculturalBankDO.getAmountIncome().replace("-", "")) + "");
                                            }
                                        }
                                ...
                                if (str.equals(merchantsBankDO.getAccountTitle()) && merchantsBankDO.equals(merchantsBankDO.getPayerPayeeName())) {
                                    if (set1.add(uuid)) {
                                        if (merchantsBankDO.getDebitAmount() != null && merchantsBankDO.getDebitAmount().length() > 0) {
                                            for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
                                                if (incomeCategoryDO.getCategoryName().equals(receivingMappingDO.getReceivingChannel())) {
                                                    incomeCategoryDO.setSouthCategoryValue(Double.parseDouble(incomeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(merchantsBankDO.getCreditAmount().replace("-", "")) + "");
                                                }
                                            }
                                ...
                                if (economicConstructionBankDO.getAccountTitle().equals(str) && economicConstructionBankDO.getCounterAccount().equals(receivingMappingDO.getOtherAccountName())) {
                                    if (set1.add(uuid)) {
                                        if (economicConstructionBankDO.getDebitAmount() != null && economicConstructionBankDO.getDebitAmount().length() > 0) {
                                            for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
                                                if (incomeCategoryDO.getCategoryName().equals(receivingMappingDO.getReceivingChannel())) {
                                                    BigDecimal originalvalue = new BigDecimal(incomeCategoryDO.getSouthCategoryValue());
                                                    BigDecimal incrementvalue = new BigDecimal(economicConstructionBankDO.getCreditAmount().replace("-", ""));
                                                    Double tempSum = originalvalue.add(incrementvalue).doubleValue();
                                                    incomeCategoryDO.setSouthCategoryValue(tempSum+ "");
                                                }
                                            }
                                        ...
                                        if (minshengBankDO.getDebitAmount() != null && minshengBankDO.getDebitAmount().length() > 0) {
                                            for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
                                                if (incomeCategoryDO.getCategoryName().equals(receivingMappingDO.getOtherAccountName())) {
                                                    incomeCategoryDO.setSouthCategoryValue(Double.parseDouble(incomeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(minshengBankDO.getCreditAmount().replace("-", "")) + "");
                                                }
                                            }
                                ...
                                if (industrialAndCommercialBankDO.getAccountTitle().equals(str) && industrialAndCommercialBankDO.getOtherPartyCompanyName().equals(receivingMappingDO.getOtherAccountName())) {
                                    if (set1.add(uuid)) {
                                        if (industrialAndCommercialBankDO.getDebitAmount() != null && industrialAndCommercialBankDO.getDebitAmount().length() > 0) {
                                            for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
                                                if (incomeCategoryDO.getCategoryName().equals(receivingMappingDO.getReceivingChannel())) {
                                                    incomeCategoryDO.setSouthCategoryValue(Double.parseDouble(incomeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(industrialAndCommercialBankDO.getCreditAmount().replace("-", "")) + "");
                                                }
                                            }
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            map1.put("南区", Double.parseDouble(map1.get("南区")) + sum + "");
        }
        for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
            incomeCategoryDO.setSouthCategoryValue(decimalFormat.format(Double.parseDouble(incomeCategoryDO.getSouthCategoryValue())));
        }
        //        ============================================给收款种类每三位加一个逗号=================================================
        for (IncomeCategoryDO incomeCategoryDO : incomeCategoryDOS) {
            try {
...
//        System.out.println(JSON.toJSONString(incomeCategoryDOS));
        ...
        map1 = AddIncomeColumnMethod(map1,"南区");
...
//======================================读取表中的数据+++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    @Override
    public Map<String, String> getSouthChargeByType(String startTime, String endTime,List<ChargeCategoryDO> chargeCategoryDOS) {
        List<AgriculturalBankDO> agriculturalBankDOList = agriculturalBankService.findByDate(startTime, endTime);
        List<ChinaBankDO> chinaBankDOList = chinaBankService.findByDate(startTime, endTime);
        List<EconomicConstructionBankDO> economicConstructionBankDOList = economicConstructionBankService.findByDate(startTime, endTime);
        List<IndustrialAndCommercialBankDO> industrialAndCommercialBankDOList = industrialAndCommercialBankService.findByDate(startTime, endTime);
        ...
        for (String str : set) {
            ...
            for (PaymentMappingDO paymentMappingDO : list) {
                try {
                    if (str.equals(paymentMappingDO.getLegelmajority()) && paymentMappingDO.getExpendproject().length()>0) {
                                    ...
                                    if (set1.add(uuid)) {
                                        for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
                                            if (chargeCategoryDO.getCategoryName().equals(paymentMappingDO.getExpendproject())) {
                                                chargeCategoryDO.setSouthCategoryValue(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(chinaBankDO.getTradeAmount().replace("-", "")) + "");
                                            }
                                        }
                                        sum = sum + Double.parseDouble(chinaBankDO.getTradeAmount().replace("-", ""));
                                    }
                            ...
                            for (AgriculturalBankDO agriculturalBankDO : agriculturalBankDOList) {
                                ...
                                if (str.equals(agriculturalBankDO.getAccountTitle()) && agriculturalBankDO.getOtherPartyName().equals(paymentMappingDO.getOtheraccountname())) {
                                    if (set1.add(uuid)) {
                                        for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
                                            if (chargeCategoryDO.getCategoryName().equals(paymentMappingDO.getExpendproject())) {
                                                chargeCategoryDO.setSouthCategoryValue(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(agriculturalBankDO.getAmountCost().replace("-", "")) + "");
                                            }
                                        }
                                ...
                                if (str.equals(merchantsBankDO.getAccountTitle()) && merchantsBankDO.equals(merchantsBankDO.getPayerPayeeName())) {
                                    if (merchantsBankDO.getDebitAmount() != null && merchantsBankDO.getDebitAmount().length() > 0) {
                                        ...
                                        if (set1.add(uuid)) {
                                            for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
                                                if (chargeCategoryDO.getCategoryName().equals(paymentMappingDO.getExpendproject())) {
                                                    chargeCategoryDO.setSouthCategoryValue(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(merchantsBankDO.getDebitAmount().replace("-", "")) + "");
                                                }
                                            }
                                ...
                                if (economicConstructionBankDO.getAccountTitle().equals(str) && economicConstructionBankDO.getCounterAccount().equals(paymentMappingDO.getOtheraccountname())) {
                                    if (set1.add(uuid)) {
                                        if (economicConstructionBankDO.getDebitAmount() != null && economicConstructionBankDO.getDebitAmount().length() > 0) {
                                            for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
                                                if (chargeCategoryDO.getCategoryName().equals(paymentMappingDO.getExpendproject())) {
                                                    chargeCategoryDO.setSouthCategoryValue(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(economicConstructionBankDO.getDebitAmount().replace("-", "")) + "");
                                                }
                                            }
                                ...
                                if (minshengBankDO.getAccountTitle().equals(str) && minshengBankDO.getOtherAccounts().equals(paymentMappingDO.getOtheraccountname())) {
                                    ...
                                    if (minshengBankDO.getDebitAmount() != null && minshengBankDO.getDebitAmount().length() > 0) {
                                        if (set1.add(uuid)) {
                                            for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
                                                if (chargeCategoryDO.getCategoryName().equals(paymentMappingDO.getExpendproject())) {
                                                    chargeCategoryDO.setSouthCategoryValue(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(minshengBankDO.getDebitAmount().replace("-", "")) + "");
                                                }
                                            }
                                ...
                                if (industrialAndCommercialBankDO.getAccountTitle().equals(str) && industrialAndCommercialBankDO.getOtherPartyCompanyName().equals(paymentMappingDO.getOtheraccountname())) {
                                    if (industrialAndCommercialBankDO.getDebitAmount() != null && industrialAndCommercialBankDO.getDebitAmount().length() > 0) {
                                        if (set1.add(uuid)) {
                                            for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
                                                if (chargeCategoryDO.getCategoryName().equals(paymentMappingDO.getExpendproject())) {
                                                    chargeCategoryDO.setSouthCategoryValue(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue()) + Double.parseDouble(industrialAndCommercialBankDO.getDebitAmount().replace("-", "")) + "");
                                                }
                                            }
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            Double sum = Double.parseDouble(entry.getValue());
            map1.put("南区", decimalFormat.format(Double.parseDouble(map1.get("南区")) + sum));
        }
        for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
            chargeCategoryDO.setSouthCategoryValue(decimalFormat.format(Double.parseDouble(chargeCategoryDO.getSouthCategoryValue())));
        }
        map1.put("总计", decimalFormat.format(Double.parseDouble(map1.get("南区"))));
//        ============================================给付款种类每三位加一个逗号=================================================
        for (ChargeCategoryDO chargeCategoryDO : chargeCategoryDOS) {
            try {
        ...
        map1 = AddChargeColumnMethod(map1,"南区");
        System.out.println(JSON.toJSONString(map1, true));
        return map1;
    }
}
